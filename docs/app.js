// app.js — Minesweeper with exact equilateral-triangle vertex lattice (no gaps)
// Supports: square, equilateral-triangle (vertex lattice), flat-top hex
// Expects HTML IDs: appRoot, msRows, msCols, msMines, tilingSelect, adjacencySelect, applyAdjacency, newGame, msStatus
// Optional sliders: triShrinkSlider (pixels), triShrinkValue (display)
...
const TILINGS = {
  ...
  triangle_equi: { label: "Equilateral Triangle", adjacencies: { "triE-edge": { label: "Edges (3)", offsets: null }, "triE-edge+v": { label: "Edges+vertices (6)", offsets: null }, "triE-r2": { label: "Radius 2", offsets: (function(){ const o=[]; for(let dr=-2;dr<=2;dr++) for(let dc=-2;dc<=2;dc++) if(!(dr===0&&dc===0)) o.push([dr,dc]); return o; })() } } },
...
function triangleEquiOffsets(r,c,adjKey){
  // adjacency for equilateral tile grid handled per-cell for edge-only options
  const up = ((r + c) % 2) === 0;
...
function getOffsetsFor(tilingKey, adjacencyKey){
  if (tilingKey === 'triangle_equi') return null;
...
function placeMines(grid, mineCount, tilingKey, adjacencyKey, safeCell = null){
  ...
  if (safeCell){
    ...
    const offs = (tilingKey === 'triangle_equi') ? triangleEquiOffsets(sr,sc,adjacencyKey).concat([[0,0]]) : getOffsetsFor(tilingKey, adjacencyKey).concat([[0,0]]);
...
function countFlaggedNeighbors(grid,r,c,tilingKey,adjacencyKey){ let offs = (tilingKey === 'triangle_equi') ? triangleEquiOffsets(r,c,adjacencyKey) : getOffsetsFor(tilingKey, adjacencyKey); let cnt=0; for (const [dr,dc] of offs){ const rr=r+dr, cc=c+dc; if (!inBounds(grid.rows,grid.cols,rr,cc)) continue; if (grid.cells[idx(grid.rows,grid.cols,rr,cc)].flagged) cnt++; } return cnt; }
function revealUnflaggedNeighbors(grid,r,c,tilingKey,adjacencyKey){ let offs = (tilingKey === 'triangle_equi') ? triangleEquiOffsets(r,c,adjacencyKey) : getOffsetsFor(tilingKey, adjacencyKey); const toReveal=[]; for (const [dr,dc] of offs){ const rr=r+dr, cc=c+dc; if (!inBounds(grid.rows,grid.cols,rr,cc)) continue; const cell = grid.cells[idx(grid.rows,grid.cols,rr,cc)]; if (!cell.flagged && !cell.revealed) toReveal.push([rr,cc]); } return toReveal; }
...
// Equilateral vertex lattice builder (exact hex-pattern)
function buildEquilateralVertexLattice(rows, cols, s, PAD = 8){
  // side length s, vertex vertical spacing h, horizontal step between vertices = s/2
  const h = Math.sqrt(3)/2 * s;
  const vertexRows = rows + 1;            // covers triangle rows
  const vertexCols = cols * 2 + 1;        // ensure enough columns since we use half-step horizontal grid
  const vertices = [];
  ...
  // Map triangles (r,c) to vertices: baseVc = c*2
  const triIndex = [];
  for (let r=0; r<rows; r++){
    for (let c=0; c<cols; c++){
      const vr = r;
...
function renderTiledBoard(){
  ...
  // square and hex fallback (center-based)
  let centersInfo;
  ...
  for (const cellInfo of centersInfo.centers){
    const r = cellInfo.r, c = cellInfo.c; const cx = cellInfo.x, cy = cellInfo.y;
    let pts;
    if (currentTiling === 'hex') pts = computeHexPolygon(cx, cy, baseSize/2, 1.0);
...
function populateTilingControls(){
  ...
  sel.innerHTML = ''; for (const key of Object.keys(TILINGS)){ const opt=document.createElement('option'); opt.value=key; opt.textContent=TILINGS[key].label; sel.appendChild(opt); }
  ...
  const tilingHandler = function(e){
    ...
    const statusEl=document.getElementById('msStatus'); if (statusEl){ const label=(TILINGS[currentTiling]&&TILINGS[currentTiling].label)||currentTiling; const adjLabel=(TILINGS[currentTiling]&&TILINGS[currentTiling].adjacencies&&TILINGS[currentTiling].adjacencies[currentAdjacency]&&TILINGS[currentTiling].adjacencies[currentAdjacency].label)||currentAdjacency; statusEl.textContent=`Tiling: ${label} (Adjacency: ${adjLabel})`; }
