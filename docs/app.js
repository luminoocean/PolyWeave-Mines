// app.js — Complete Minesweeper with Square, Hex, Right-Triangle, and Equilateral-Triangle tilings
// Robust DOM-by-ID wiring, vertex-lattice triangle renderers, temporary sliders for tuning
...
// --- Adjacency helpers for triangle logic ---
function triangleRightOffsets(r,c,adjKey){
  const right = ((r+c) % 2) === 0; // parity -> orientation
...
function triangleEquiOffsets(r,c,adjKey){
  ...
  if (adjKey === 'triE-vert') { const arr=[]; for(let dr=-1;dr<=1;dr++) for(let dc=-1;dc<=1;dc++) if(!(dr===0&&dc===0)) arr.push([dr,dc]); return arr; }
...
// --- Vertex-lattice builders and shrink helpers ---

// Right-triangle lattice: subdivide squares into two right triangles per square cell
function buildRightTriangleLattice(rows, cols, s, PAD=8){
  const step = s; // we use step = s for grid spacing so triangles side ~ s
  const vrows = rows + 1;
  const vcols = cols + 1;
  const vertices = [];
...
// Equilateral triangle lattice: build standard triangular/hex lattice
function buildEquilateralVertexLattice(rows, cols, s, PAD=8){
  const h = Math.sqrt(3)/2 * s;
  // vertex rows = rows + 1
  // horizontal step between adjacent vertex positions = s/2
  const vertexRows = rows + 1;
  const vertexCols = cols * 2 + 2; // generous count to cover mapping
  const vertices = [];
  ...
  const vertexColsEffective = vertexCols;
  const vIndex = (vr,vc) => Math.max(0, Math.min(vertexRows-1, vr)) * vertexColsEffective + Math.max(0, Math.min(vertexColsEffective-1, vc));
  const triIndex = [];
  for (let r=0;r<rows;r++){
    for (let c=0;c<cols;c++){
      const vr = r;
...
function renderTiledBoard(){
  ...
  const baseSize = Math.max(12, Math.min(48, rawBase));
  ...
  if (tileType === 'triangle_right'){
    ...
    for (const ti of triIndex){
      ...
      poly.addEventListener('click', ()=> {
        ...
        if (res.exploded) {
          running = false; gameGrid.cells.forEach(cl => { if (cl.mine) cl.revealed = true; }); const ms = document.getElementById('msStatus'); if (ms) ms.textContent = 'BOOM — you hit a mine';
  ...
  // hex or square fallback (center-based)
  let centersInfo;
...
// square and hex center helpers (used above)
...
function populateTilingControls(){
  ...
  const tilingHandler = function(e){
    ...
    if (statusEl) { const label = (TILINGS[currentTiling] && TILINGS[currentTiling].label) || currentTiling; const adjLabel = (TILINGS[currentTiling] && TILINGS[currentTiling].adjacencies && TILINGS[currentTiling].adjacencies[currentAdjacency] && TILINGS[currentTiling].adjacencies[currentAdjacency].label) || currentAdjacency; statusEl.textContent = `Tiling: ${label} (Adjacency: ${adjLabel})`; }
    if (gameGrid && typeof computeCountsWithAdjacency === 'function'){ try { computeCountsWithAdjacency(gameGrid, currentTiling, currentAdjacency); renderTiledBoard(); } catch(err){ console.error(err); startNewGame(); } } else startNewGame();
...
function applyAdjacencyAction(){
  ...
  currentTiling = sel.value; currentAdjacency = adjSel.value; try{ window.currentTiling=currentTiling; window.currentAdjacency=currentAdjacency; }catch(e){}
  if (gameGrid){ computeCountsWithAdjacency(gameGrid, currentTiling, currentAdjacency); renderTiledBoard(); const ms=document.getElementById('msStatus'); if (ms) ms.textContent = `Applied ${TILINGS[currentTiling].label} + ${TILINGS[currentTiling].adjacencies[currentAdjacency].label}`; }
  else { const ms=document.getElementById('msStatus'); if (ms) ms.textContent = `Applied ${TILINGS[currentTiling].label} + ${TILINGS[currentTiling].adjacencies[currentAdjacency].label}`; }
}
...
function initOnceDomReady(){
  ...
  const xSlider = document.getElementById('xGapSlider'); const ySlider = document.getElementById('yGapSlider'); const triSlider = document.getElementById('triShrinkSlider');
  if (xSlider) xSlider.addEventListener('input', (e)=>{ const v=e.target.value; const el=document.getElementById('xGapValue'); if (el) el.textContent=v; renderTiledBoard(); });
  if (ySlider) ySlider.addEventListener('input', (e)=>{ const v=e.target.value; const el=document.getElementById('yGapValue'); if (el) el.textContent=v; renderTiledBoard(); });
