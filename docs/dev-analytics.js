(function DevAnalyticsModule() {
  // dev-analytics.js
  // Developer Analytics for Polyweave Mines (single-file, robust, safe to include multiple times)
  ...
  // Expose safely (do not overwrite if already present)
  if (!window.DevAnalytics) {
    window.DevAnalytics = DevAnalytics;
  } else {
    // merge minimal functions if existing object is present
    try {
      Object.assign(window.DevAnalytics, {
        isDevMode: window.DevAnalytics.isDevMode || isDevMode,
        recheckAndInit: window.DevAnalytics.recheckAndInit || function() {
  ...
  if (!window.DevAnalytics || !window.DevAnalytics.recheckAndInit) {
    (window.DevAnalytics || (window.DevAnalytics = {}));
    window.DevAnalytics.recheckAndInit = function recheckAndInit() {
      if (isDevMode() && !analyticsInitialized) {
        ensureStorage();
        startSession();
        attachHooks();
